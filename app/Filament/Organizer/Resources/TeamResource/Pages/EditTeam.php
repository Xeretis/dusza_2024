...
class EditTeam extends EditRecord
{
    ...
    protected function mutateFormDataBeforeFill(array $data): array
    {
        ...
        $teachers = CompetitorProfile::where('type', CompetitorProfileType::Teacher)->whereHas('teams', function ($query) use ($data) {
            $query->where('teams.id', $data['id']);
        })->get()->map(function ($p) {
            return ['id' => $p->id];
        })->values();
    ...
    protected function handleRecordUpdate(Model $record, array $data): Model
    {
        ...
        $record->update(collect($data)->forget(['competitor1', 'competitor2', 'competitor3', 'teachers'])->toArray());
