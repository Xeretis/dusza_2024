...
class ViewTeamEvents extends Page
{
    protected static string $resource = TeamEventResource::class;

    protected static string $view = 'filament.competitor.resources.team-event-resource.pages.view-team-events';
        ...
        return [
            'activity_section' => [
                ...
                'empty_state_description' =>
                    'Check back later for activities that have been recorded.',
    ...
    {
        $activityTitle = $this->modifiedState()['activity_title'][
            'modify_state'
        ];
        $activityDescription = $this->modifiedState()['activity_description'][
            'modify_state'
        ];

        if (isset($this->configuration()['activity_title']['modify_state'])) {
            $activityTitle = $this->configuration()['activity_title'][
                'modify_state'
            ];
        }
        ...
        if (
            isset(
                $this->configuration()['activity_description']['modify_state']
            )
        ) {
            $activityDescription = $this->configuration()[
                'activity_description'
            ]['modify_state'];
        }
        ...
        return $infolist
            ...
            ->schema([
                ActivitySection::make('activities')
                    ...
                    ->description(
                        $this->configuration()['activity_section'][
                            'description'
                        ]
                    )
                    ->schema([
                        ActivityTitle::make('activityData')
                            ->placeholder(
                                $this->configuration()['activity_title'][
                                    'placeholder'
                                ]
                            )
                            ->allowHtml(
                        ...
                        ActivityDescription::make('activityData')
                            ->placeholder(
                                $this->configuration()['activity_description'][
                                    'placeholder'
                                ]
                            )
                            ->allowHtml(
                        ...
                        ActivityDate::make(
                            $this->configuration()['activity_date']['name']
                        )
                            ->date(
                                $this->configuration()['activity_date']['date']
                            )
                            ->placeholder(
                                $this->configuration()['activity_date'][
                                    'placeholder'
                                ]
                            ),
                        ActivityIcon::make('event')
                            ->icon(
                                $this->configuration()['activity_icon']['icon']
                            )
                            ->color(
                                $this->configuration()['activity_icon']['color']
                            ),
                    ])
                    ->showItemsCount(
                        $this->configuration()['activity_section'][
            ...
            'activity_title' => [
                'modify_state' => function (array $state) {
                    if ($state['description'] == $state['event']) {
                        ...
                        $causerName =
                            $state['causer']->name ??
                            ($state['causer']->first_name ??
                                ($state['causer']->last_name ??
                                    ($state['causer']->username ?? 'Unknown')));

                        return new HtmlString(
                            sprintf(
                                'The <strong>%s</strong> was <strong>%s</strong> by <strong>%s</strong>.',
                                $className,
                                $state['event'],
                                $causerName
                            )
                ...
                'modify_state' => function (array $state) {
                    ...
                    if (
                        ...
                        foreach ($newValues as $key => $newValue) {
                            $oldValue = is_array($oldValues[$key])
